...
public class CellScript : MonoBehaviour
{
    ...
    void GribMove()
    {
        if (parent.transform.localPosition.x > -3)
            parent.localPosition -= new Vector3(Time.smoothDeltaTime * 8, 0, 0);
        else
        {
            parent.localPosition = new Vector3(-3, parent.localPosition.y, parent.localPosition.z);
            movedone = true;
        }
    }
    /// <summary>
    ...
    int[,] mapReader(string mapname)
    {
        ...
        string mapdata = "";
        string assetpath = @"Assets/Minecrash/Resources/Maps/" + mapname + ".txt";
#if UNITY_EDITOR
